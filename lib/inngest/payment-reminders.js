    ...
    const results = await step.run("send-emails", async () => {
      return Promise.all(
        users.map(async (u) => {
          ...
          if (!rows) {
            console.warn(`No debts for user ${u._id} (${u.name}) — skipping`);
            return { userId: u._id, skipped: true };
          }

          if (!u.email) {
            console.warn(`Skipping user ${u._id} (${u.name}) — missing email`);
            return { userId: u._id, skipped: true, error: "Missing email" };
          }
          ...
          try {
            // Note: we no longer pass apiKey here; Convex action reads it from server env
            await convex.action(api.email.sendEmail, {
              to: u.email,
              subject: "You have pending payments on Splitr",
              html,
            });
            console.log(`Queued email send for user ${u._id} (${u.email})`);
            return { userId: u._id, success: true };
          } catch (err) {
            console.error("sendEmail action error for user", u._id, err);
            return { userId: u._id, success: false, error: err?.message || String(err) };
          }
        })
      );
    });
